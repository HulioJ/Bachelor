...
#linear regression
    ...
    #proximal distances
      ...
      #assumption checks
      qqnorm(df$thisthatresp)
      
     hyp1 <-  aov(thisthatresp~Trial*Condition+(1|participant), data = df)
     summary(hyp1)

     
   hypx <-   lmerTest::lmer(sentiment~Trial*Condition + (1|participant), data = df)
      summary(hypx)
      
      pos_context <- subset(df, Condition == 1)
      neg_context <- subset(df, Condition == 0)
           
      #Sentiment change Post versus Pre trial, each condition
      
   hyp3 <-   lmerTest::lmer(sentiment_scaled~Trial + (1|participant), data = pos_context)
      summary(hyp3)
   hyp3.1 <-   lmerTest::lmer(sentiment_scaled~Trial + (1|participant), data = neg_context)
      summary(hyp3.1)

      
    #Only slef sentiments
      
      pos_context_self <- subset(pos_context, type == "self")
      neg_context_self <- subset(neg_context, type == "self")
      
      hyp4 <-   lmerTest::lmer(sentiment_scaled~Trial + (1|participant), data = pos_context_self)
        summary(hyp4)
      hyp4.1 <-   lmerTest::lmer(sentiment_scaled~Trial + (1|participant), data = neg_context_self)
        summary(hyp4.1)
